      ...
      <el-header height="45px">
        <el-form :inline="true" :model="formInline" label-width="auto;" class="form">
          <el-form-item label="网站名字" prop="web_id">
            <el-select v-model="formInline.web_id" placeholder="请选择网站名字" @change="getMode" clearable filterable>
          ...
          <el-form-item label="模块名字" prop="module_id">
            <el-select v-model="formInline.module_id" placeholder="请选择模块名字" clearable filterable :disabled="editData">
        ...
        <el-table ref="singleTableRef" :data="tableData" highlight-current-row style="width: 100%" border
          ...
          <el-table-column fixed="right" label="操作" width="200">
            <template #default="scope">
              ...
              <el-button size="small" @click.prevent="deleteRow(scope.row.column_id)" type="danger">
                删除
              </el-button>
            </template>
          </el-table-column>
...
import { ref } from "vue";
import MainDialog from "@/components/dialog/websiteColumn.vue";
import {
  updateWebCol,
  WebColList,
  deleteWebCol,
...
const getMode = async (value) => {
  if (value) {
    ...
    moduleOptions.value = [];
    formInline.value.module_id = '';
  }
};

/* 删除操作 */
const deleteRow = async (column_id: number) => {
  ElMessageBox.confirm("您确定要删除这个栏目?", "提示", {
    ...
    .then(async () => {
      ...
      let data = await deleteWebCol(params);
